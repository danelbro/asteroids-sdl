...
{
    textObjects.push_back(TextObject{gw, pos, font, customCols::text_col, renderer});
    textObjects[0].updateText("Score: ", m_renderer);

    textObjects.push_back(TextObject{gw, { pos.x + textObjects[0].size().x, pos.y},
                                     font, customCols::text_col, renderer});
    textObjects[1].updateText(std::to_string(score), m_renderer);

    textObjects.push_back(TextObject{gw, {pos.x, pos.y + textObjects[0].size().y},
                                     font, customCols::text_col, renderer});
    textObjects[2].updateText("Lives: ", m_renderer);

    textObjects.push_back(TextObject{gw, {pos.x + textObjects[2].size().x, textObjects[2].pos().y},
                                     font, customCols::text_col, renderer});
